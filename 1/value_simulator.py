...
class LQR_simulator:
    ...
    def opt_control(self, S, X):
        '''
        Compute the optimal control a = -D^{-1}M^{T}S(t)x, where
        s: torch tensor, dim = 2*2
        x: torch tensor, dim = batchsize*2*1
        '''
        return -torch.inverse(self.D) @ self.M.t() @ S @ X
